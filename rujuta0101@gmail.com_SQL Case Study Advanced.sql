...
--1. All states which hv customers who have bought cellphones from 2005 till today

select distinct A.state 
from DIM_LOCATION A, FACT_TRANSACTIONS B, DIM_DATE C
where A.IDLocation	=B.IDLOCATION
AND C.date=B.date
AND  c.year > '2005'
...
select top 1 A.state, COUNT(B.QUANTITY) Samsung_models_sold_in_US
from DIM_LOCATION A, FACT_TRANSACTIONS B, DIM_MANUFACTURER C, DIM_MODEL D
WHERE  A.IDLOCATION=B.IDLOCATION
AND B.IDMODEL=D.IDMODEL
AND D.IDMANUFACTURER=C.IDMANUFACTURER
GROUP BY A.STATE


--3.NO OF Transactions for each model per zipcode per state CAST(A.DATE AS VARCHAR)
select  B.zipcode, B.state, C.model_name,  count (A.IDMODEL+ A.idcustomer + A.IDLOCATION  ) No_Of_Transaction
FROM FACT_TRANSACTIONS A, DIM_LOCATION B, DIM_MODEL C
WHERE A.IDLOCATION=B.IDLOCATION
AND C.IDMODEL=A.IDMODEL
GROUP BY   B.zipcode, B.state, C.model_name


--4. SHow the cheapest cell phone
select top 1 A.unit_price, A.model_name, b.Manufacturer_name
from DIM_MODEL A, DIM_MANUFACTURER B
WHERE A.IDMANUFACTURER= B.IDMANUFACTURER
ORDER BY A.unit_price

--5.AVg price for each model in top 5 manufacturer in term of sales quantity and  order by avg price
SELECT C1.IDMODEL ,b1.MANUFACTURER_name, SUM(A1.totalprice)/SUM(A1.QUANTITY) Avg_Price
FROM FACT_TRANSACTIONS A1, DIM_MODEL C1, DIM_MANUFACTURER B1
WHERE A1.IDMODEL=C1.IDMODEL
AND B1. IDMANUFACTURER = C1.IDMANUFACTURER
AND C1.IDMANUFACTURER IN (
select TOP 5 A.IDMANUFACTURER
FROM DIM_MANUFACTURER A, FACT_TRANSACTIONS B, DIM_MODEL C
WHERE A.IDMANUFACTURER=C.IDMANUFACTURER
AND C.IDMODEL=B.IDMODEL
GROUP BY A.IDMANUFACTURER
ORDER BY COUNT(B.QUANTITY) DESC)
GROUP BY C1.IDMODEL, b1.MANUFACTURER_name
order by SUM(A1.totalprice)/SUM(A1.QUANTITY)


--6. customer and avg amount spent IN 2009 above 500
select A.customer_name,YEAR(B.DATE), SUM(B.totalprice)/SUM(B.QUANTITY) AVG_SPENT
from DIM_CUSTOMER A, FACT_TRANSACTIONS B
WHERE A.IDCUSTOMER= B.IDCUSTOMER
AND YEAR(B.DATE)=2009
GROUP BY A.CUSTOMER_NAME, YEAR(B.DATE)
HAVING (SUM(B.totalprice)/SUM(B.QUANTITY)) > 500 ---No customer in 2009 with spend above 500
ORDER BY 2 DESC


--7.MODELS IN TOP 5 QTY SOLD IN 2008, 2009 AND 2010
--No model in top5 qty consistently across 2008, 2009 and 2010
SELECT Z.IDMODEL FROM 
(
	SELECT A1.IDMODEL FROM
	(
		SELECT TOP 5 A.IDMODEL
	...
	INNER JOIN 
	(
	SELECT TOP 5 B.IDMODEL
	FROM FACT_TRANSACTIONS B
	WHERE YEAR(B.DATE)= 2010
	GROUP BY B.IDMODEL
	ORDER BY SUM(B.QUANTITY)
	)B1
ON A1.IDMODEL=B1.IDMODEL
) Z
INNER JOIN
(
SELECT TOP 5 C.IDMODEL
FROM FACT_TRANSACTIONS C
WHERE YEAR(C.DATE)= 2008
GROUP BY C.IDMODEL
ORDER BY SUM(C.QUANTITY)
)Y
ON Z.IDMODEL=Y.IDMODEL


--8 Manufacturers with 2nd top sales in 2009 and 2010  

SELECT Manufacturer_Name, YEAR,TOTAL FROM
(select * from
(select TOP 2 A.Manufacturer_Name , YEAR(B.DATE) AS YEAR,sum(B.TOTALPRICE) AS TOTAL, ROW_NUMBER() OVER (ORDER BY sum(B.TOTALPRICE)) ROWNUM
FROM DIM_MANUFACTURER A, FACT_TRANSACTIONS B, DIM_MODEL C
WHERE C.IDMODEL= B.IDMODEL
AND C.IDMANUFACTURER= A.IDMANUFACTURER
AND YEAR(B.DATE)= 2009
GROUP BY A.Manufacturer_Name,YEAR(B.DATE)
ORDER BY 2 DESC 
) A
WHERE ROWNUM=2
) A
...
SELECT Manufacturer_Name, yEAR,tOTAL FROM
(
select TOP 2 A1.Manufacturer_Name ,YEAR(B1.DATE) AS YEAR, sum(B1.TOTALPRICE) AS TOTAL, ROW_NUMBER() OVER (ORDER BY sum(B1.TOTALPRICE)) ROWNUM
FROM DIM_MANUFACTURER A1, FACT_TRANSACTIONS B1, DIM_MODEL C1
WHERE C1.IDMODEL= B1.IDMODEL
AND C1.IDMANUFACTURER= A1.IDMANUFACTURER
AND YEAR(B1.DATE)= 2010
GROUP BY A1.Manufacturer_Name,YEAR(B1.DATE)
ORDER BY 2 DESC 
)B 
WHERE ROWNUM=2
;
...
--9. Manufactures who sold in 2010 but not in 2009
select DISTINCT A.manufacturer_name
from dim_manufacturer A, DIM_MODEL B, FACT_TRANSACTIONS C
where A.IDMANUFACTURER=B.IDMANUFACTURER
AND B.IDMODEL= C.IDMODEL
AND YEAR(C.DATE)= 2010
AND a.manufacturer_name NOT IN (
select DISTINCT A1.manufacturer_name
from dim_manufacturer A1, DIM_MODEL B1, FACT_TRANSACTIONS C1
where A1.IDMANUFACTURER=B1.IDMANUFACTURER
AND B1.IDMODEL= C1.IDMODEL
AND YEAR(C1.DATE)= 2009
)

--10 top 100 customers and their avg spend, avg quantity by each year .
--Also find percentage of change in their spend

select u.CUSTOMER_NAME, 
u.Avg_Spend_2003,
(case when u.Avg_Spend_2003= '0' then 0 else ((u.Avg_Spend_2004-u.Avg_Spend_2003)/u.Avg_Spend_2003*100)  end)ChangeIn2004,
u.Avg_Spend_2004,
 (case when u.Avg_Spend_2004= '0' then 0 else ((u.Avg_Spend_2005-u.Avg_Spend_2004)/u.Avg_Spend_2004*100)  end)ChangeIn2005,
 u.Avg_Spend_2005,
 (case when u.Avg_Spend_2005= '0' then 0 else ((u.Avg_Spend_2006-u.Avg_Spend_2005)/u.Avg_Spend_2005*100)  end)ChangeIn2006,
 u.Avg_Spend_2006,
 (case when u.Avg_Spend_2006= '0' then 0 else ((u.Avg_Spend_2007-u.Avg_Spend_2006)/u.Avg_Spend_2006*100)  end)ChangeIn2007,
 u.Avg_Spend_2007,
 (case when u.Avg_Spend_2007= '0' then 0 else ((u.Avg_Spend_2008-u.Avg_Spend_2007)/u.Avg_Spend_2007*100)  end)ChangeIn2008,
 u.Avg_Spend_2008,
 (case when u.Avg_Spend_2008= '0' then 0 else ((u.Avg_Spend_2009-u.Avg_Spend_2008)/u.Avg_Spend_2008*100)  end)ChangeIn2009,
 u.Avg_Spend_2009,
 (case when u.Avg_Spend_2009= '0' then 0 else ((u.Avg_Spend_2010-u.Avg_Spend_2009)/u.Avg_Spend_2009*100)  end)ChangeIn2010,
 u.Avg_Spend_2010,
 u.Avg_Qty_2003,
  u.Avg_Qty_2004,
   u.Avg_Qty_2005,
...
(	
	...
	CONVERT(DECIMAL(5,2),(CASE WHEN T.Q_2003='0' THEN 0 ELSE (T.TP_2003/T.Q_2003) END)) Avg_Spend_2003,
	(CASE WHEN T.Qc_2003='0' THEN 0 ELSE (CAST(T.Q_2003 AS FLOAT)/CAST(T.Qc_2003 AS FLOAT)) END) Avg_Qty_2003,
	(CASE WHEN T.Q_2004='0' THEN 0 ELSE (T.TP_2004/T.Q_2004) END) Avg_Spend_2004,
	CONVERT(DECIMAL(5,2),(CASE WHEN T.Qc_2004='0' THEN 0 ELSE (CAST(T.Q_2004 AS FLOAT)/CAST(T.Qc_2004 AS FLOAT)) END)) Avg_Qty_2004,
	(CASE WHEN T.Q_2005='0' THEN 0 ELSE (T.TP_2005/T.Q_2005) END) Avg_Spend_2005,
	CONVERT(DECIMAL(5,2),(CASE WHEN T.Qc_2005='0' THEN 0 ELSE (CAST(T.Q_2005 AS FLOAT)/CAST(T.Qc_2005 AS FLOAT)) END)) Avg_Qty_2005,
	(CASE WHEN T.Q_2006='0' THEN 0 ELSE (T.TP_2006/T.Q_2006) END) Avg_Spend_2006,
	CONVERT(DECIMAL(5,2),(CASE WHEN T.Qc_2006='0' THEN 0 ELSE (CAST(T.Q_2006 AS FLOAT)/CAST(T.Qc_2006 AS FLOAT)) END)) Avg_Qty_2006,
	(CASE WHEN T.Q_2007='0' THEN 0 ELSE (T.TP_2007/T.Q_2007) END) Avg_Spend_2007,
	(CASE WHEN T.Qc_2007='0' THEN 0 ELSE (CAST(T.Q_2007 AS FLOAT)/CAST(T.Qc_2007 AS FLOAT)) END) Avg_Qty_2007,
	(CASE WHEN T.Q_2008='0' THEN 0 ELSE (T.TP_2008/T.Q_2008) END) Avg_Spend_2008,
	(CASE WHEN T.Qc_2008='0' THEN 0 ELSE (CAST(T.Q_2008 AS FLOAT)/CAST(T.Qc_2008 AS FLOAT)) END) Avg_Qty_2008,
	(CASE WHEN T.Q_2009='0' THEN 0 ELSE (T.TP_2009/T.Q_2009) END) Avg_Spend_2009,
	(CASE WHEN T.Qc_2009='0' THEN 0 ELSE (CAST(T.Q_2009 AS FLOAT)/CAST(T.Qc_2009 AS FLOAT)) END) Avg_Qty_2009,
	(CASE WHEN T.Q_2010='0' THEN 0 ELSE (T.TP_2010/T.Q_2010) END) Avg_Spend_2010,
	(CASE WHEN T.Qc_2010='0' THEN 0 ELSE (CAST(T.Q_2010 AS FLOAT)/CAST(T.Qc_2010 AS FLOAT)) END) Avg_Qty_2010
	---ADDING
	--,Q_2003, Qc_2003

	FROM(
		select TOP 100 A.customer_name, 
		SUM( CASE WHEN YEAR(B.DATE)=2003 THEN b.totalprice ELSE 0 end) as TP_2003,
		SUM( CASE WHEN YEAR(B.DATE)=2003 THEN B.QUANTITY ELSE 0 END)  as Q_2003,
		count( CASE WHEN YEAR(B.DATE)=2003 THEN B.idmodel ELSE 0 END)  as Qc_2003,
		SUM( CASE WHEN YEAR(B.DATE)=2004 THEN b.totalprice ELSE 0 end) as TP_2004,
		SUM( CASE WHEN YEAR(B.DATE)=2004 THEN B.QUANTITY ELSE 0 END)  as Q_2004,
		count( CASE WHEN YEAR(B.DATE)=2004 THEN B.QUANTITY ELSE 0 END)  as Qc_2004,
		SUM( CASE WHEN YEAR(B.DATE)=2005 THEN b.totalprice ELSE 0 end) as TP_2005,
		SUM( CASE WHEN YEAR(B.DATE)=2005 THEN B.QUANTITY ELSE 0 END)  as Q_2005,
		count( CASE WHEN YEAR(B.DATE)=2005 THEN B.QUANTITY ELSE 0 END)  as Qc_2005,
		SUM( CASE WHEN YEAR(B.DATE)=2006 THEN b.totalprice ELSE 0 end) as TP_2006,
		SUM( CASE WHEN YEAR(B.DATE)=2006 THEN B.QUANTITY ELSE 0 END)  as Q_2006,
		count( CASE WHEN YEAR(B.DATE)=2006 THEN B.QUANTITY ELSE 0 END)  as Qc_2006,
		SUM( CASE WHEN YEAR(B.DATE)=2007 THEN b.totalprice ELSE 0 end) as TP_2007,
		SUM( CASE WHEN YEAR(B.DATE)=2007 THEN B.QUANTITY ELSE 0 END)  as Q_2007,
		count( CASE WHEN YEAR(B.DATE)=2007 THEN B.QUANTITY ELSE 0 END)  as Qc_2007,
		SUM( CASE WHEN YEAR(B.DATE)=2008 THEN b.totalprice ELSE 0 end) as TP_2008,
		SUM( CASE WHEN YEAR(B.DATE)=2008 THEN B.QUANTITY ELSE 0 END)  as Q_2008,
		count( CASE WHEN YEAR(B.DATE)=2008 THEN B.QUANTITY ELSE 0 END)  as Qc_2008,
		SUM( CASE WHEN YEAR(B.DATE)=2009 THEN b.totalprice ELSE 0 end) as TP_2009,
		SUM( CASE WHEN YEAR(B.DATE)=2009 THEN B.QUANTITY ELSE 0 END)  as Q_2009,
		count( CASE WHEN YEAR(B.DATE)=2009 THEN B.QUANTITY ELSE 0 END)  as Qc_2009,
		SUM( CASE WHEN YEAR(B.DATE)=2010 THEN b.totalprice ELSE 0 end) as TP_2010,
		SUM( CASE WHEN YEAR(B.DATE)=2010 THEN B.QUANTITY ELSE 0 END)  as Q_2010,
		count( CASE WHEN YEAR(B.DATE)=2010 THEN B.QUANTITY ELSE 0 END)  as Qc_2010
		from DIM_CUSTOMER A, FACT_TRANSACTIONS B
		WHERE A.IDCUSTOMER=B.IDCUSTOMER
